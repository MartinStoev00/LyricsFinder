...
export default class Lyrics extends Component {
    ...
    componentDidMount() {
        ...
        const link = `https://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=${this.props.match.params.id}`;
        axios.get(`${prefix}${link}&apikey=${process.env.REACT_APP_MM_KEY}`)
            .then(res => {
                this.setState({lyrics: res.data.message.body.lyrics});
                return axios.get(
                    `${prefix}https://api.musixmatch.com/ws/1.1/track.get?track_id=${this.props.match.params.id}&apikey=${process.env.REACT_APP_MM_KEY}`
                )
            }).then(res => {
